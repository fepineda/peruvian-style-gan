...
if __name__ == '__main__':
    ...
    generator.load_state_dict(torch.load(args.path, map_location=torch.device('cpu')))
    generator.eval()
    ...
    utils.save_image(img, './sample_matrix/sample.png', nrow=args.n_col, normalize=True, range=(-1, 1))
    
    for j in range(20):
        img = style_mixing(generator, step, mean_style, args.n_col, args.n_row, device)
        utils.save_image(
            img, f'./sample_matrix/sample_mixing_{j}.png', nrow=args.n_col + 1, normalize=True, range=(-1, 1)
        )
